import os
os.environ["PROJECT_ROOT"] = os.path.dirname(os.path.abspath(__file__))[:-4]
...
import argparse, pickle, random, torch, json
...
def main():
    ...
    parser.add_argument("-ds", "--dataset", default="CySec/GeneralCySec",
                        help="SpecializedCySec or GeneralCySec dataset")
    ...
    os.environ['ADAPET_ROOT'] = os.path.join(os.environ["PROJECT_ROOT"], "src", "ADAPET")
    ...
    try:
        os.mkdir(os.path.join(os.environ["PROJECT_ROOT"], "Configs/"))
    except OSError as error:
        pass
    config_path = os.path.join(os.environ["PROJECT_ROOT"], "Configs", parameters["exp_name"]+".json")
    ...
    (X_train_full, y_train_full), (X_train_few, y_train_few), (X_dev_full, y_dev_full), (X_dev_few, y_dev_few), (
    X_test, y_test) = data.read_datasets.read_specialized_cysec_data()
    ...
    if str2bool(args.augmentation):
        train_pos_path = os.path.join(os.environ["PROJECT_ROOT"], "msexchange-server-cti-dataset", "augmented","SpecializedCySec", "train_pos.pkl")
        train_neg_path = os.path.join(os.environ["PROJECT_ROOT"], "msexchange-server-cti-dataset", "augmented","SpecializedCySec", "train_neg.pkl")
        files_found = os.path.isfile(
            train_pos_path) and os.path.isfile(
            train_neg_path)

        if not str2bool(args.cached_files) or not files_found:
            print(
                "Augmenting the data is not implemented! Pull the Master and try using the already augmented datasets.")
        ...
        with open(train_pos_path, 'rb') as fp:
            X_train_additional_pos = list(pickle.load(fp))
        with open(train_neg_path, 'rb') as fp:
    ...
    if args.tune == "ClassifierHead":
        ...
        (X_train_finetuning, y_train_finetuning), (X_dev_finetuning, y_dev_finetuning), (
        X_test_finetuning, y_test_finetuning) = \
        ...
        finetuned_trainer.train(X_train_finetuning, y_train_finetuning, X_dev_finetuning, y_dev_finetuning,
                                X_test_finetuning,
                                y_test_finetuning, parameters)
        model_dir = finetuned_trainer.save_model(parameters["exp_dir"])
        ...
        data.adapet_preperation.save_datasets_for_adapet("GeneralCySec",
                                                         X_train_finetuning, y_train_finetuning, [], [],
                                                         X_dev_finetuning, y_dev_finetuning, X_test_finetuning,
                                                         y_test_finetuning)
    ...
    if args.mode == "0":
        ...
        normal_trainer.save_model(parameters["exp_dir"], parameters["exp_name"])
        ...
        data.adapet_preperation.save_datasets_for_adapet("SpecializedCySec",
                                                         X_train, y_train, augmented_data_pos, augmented_data_neg,
        ...
        templated_trainer.train(X_train, y_train, X_dev, y_dev, X_test, y_test, epochs=3, bs=5, warmup_steps=30000,
                                weight_decay=0.01)
