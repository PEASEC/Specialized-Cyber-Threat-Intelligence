...
class ADAPETCySecTrainer:
    """
    WrapperClass to train with the ADAPET module
    """
    ...
    def train(self, return_model=False):
        """
        Just calls the train method from ADAPET
        """
        if return_model:
    ...
    def eval_model(self, path):
        """
        Interprets saved results from ADAPET and calculates the scores
    ...
    def evaluate(self):
        ...
        self.eval_model(os.path.join(self.config.exp_dir,"test.json"))
    
    def save_model(self):
        """
        Model is already saved by ADAPET - just returning the right folder
        :return: folder of saved model
        """
