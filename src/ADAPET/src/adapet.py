...
class adapet(torch.nn.Module):
    def __init__(self, config, tokenizer, dataset_reader):
        ...
        elif "bert" in pretrained_file:
            bert_config = BertConfig.from_pretrained(pretrained_file)
            self.model = BertForMaskedLM.from_pretrained(pretrained_file, config=bert_config)
        else:
