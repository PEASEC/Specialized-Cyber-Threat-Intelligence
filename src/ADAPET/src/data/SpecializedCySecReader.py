...
from ADAPET.src.data.tokenize import tokenize_pet_txt, tokenize_pet_mlm_txt
from ADAPET.src.utils.util import device

class SpecializedCySecReader(object):
    '''
    CySecReader reads CySec dataset
    '''
    ...
    def __init__(self, config, tokenizer):
        ...
        self.pet_patterns = [["[POST]", " Question : Is this text helpful for cybersecurity experts ? Answer : {}. [SEP]".format(self.tokenizer.mask_token), ""],
                             ["[POST]", " Based on the previous passage, is this text helpful for cybersecurity experts ? {}. [SEP]".format(self.tokenizer.mask_token), ""],
                             ["Based on the following passage, is this text helpful for cybersecurity experts ? {}. ".format(self.tokenizer.mask_token), " [POST] [SEP]", ""]]
    ...
    def _get_file(self, split):
        ...
        if split.lower() == "train":
            file = os.path.join(os.environ['ADAPET_ROOT'], "data", "CySec", "SpecializedCySec", "train.jsonl")
        elif split.lower() == "dev":
            file = os.path.join(os.environ['ADAPET_ROOT'], "data", "CySec", "SpecializedCySec", "val.jsonl")
        elif split.lower() == "test":
            file = os.path.join(os.environ['ADAPET_ROOT'], "data", "CySec", "SpecializedCySec", "test.jsonl")
        return file
    ...
    def read_dataset(self, split=None, is_eval=False):
        ...
        with open(file, 'r') as f_in:
            for i, line in enumerate(f_in.readlines()):
                ...
                dict_input = {}
                dict_input["post"] = json_string["post"]
    ...
    def prepare_pet_batch(self, batch, mode="PET1"):
        ...
        :return:
        '''
        list_post = batch["input"]["post"]

        list_input_ids = []
        bs = len(batch["input"]["post"])
    ...
    def prepare_pet_mlm_batch(self, batch, mode="PET1"):
        ...
        for b_idx, (p, lbl) in enumerate(zip(list_post, prep_lbl)):
            txt_split_tuple = []

            txt_trim = -1

            for idx, txt_split in enumerate(pattern):
                txt_split_inp = txt_split.replace("[POST]", p).replace("[MASK]", label[lbl])
