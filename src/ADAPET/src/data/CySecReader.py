...
class CySecReader(object):
    ...
    def __init__(self, config, tokenizer):
        ...
        self.pet_patterns = [["[POST]", " Question : Is this text about cybersecurity ? Answer : {}. [SEP]".format(self.tokenizer.mask_token), ""],
                             ["[POST]", " Based on the previous passage, is this text about cybersecurity ? {}. [SEP]".format(self.tokenizer.mask_token), ""],
                             ["Based on the following passage, is this text about cybersecurity ? {}. ".format(self.tokenizer.mask_token), " [POST] [SEP]", ""]]
    ...
    def _get_file(self, split):
        ...
        if split.lower() == "train":
            file = os.path.join(os.environ['ADAPET_ROOT'], "data", "CySec", "GeneralCySec", "train.jsonl")
        elif split.lower() == "dev":
            file = os.path.join(os.environ['ADAPET_ROOT'], "data", "CySec", "GeneralCySec", "val.jsonl")
        elif split.lower() == "test":
            file = os.path.join(os.environ['ADAPET_ROOT'], "data", "CySec", "GeneralCySec", "test.jsonl")
        return file
