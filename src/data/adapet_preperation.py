...
def save_datasets_for_adapet(path, X_train, y_train, X_train_augmented_pos, X_train_augmented_neg, X_dev, y_dev, X_test, y_test):
    print(os.getcwd())
    with open('./msexchange-server-cti-dataset/processed/adapet/' + path + '/val.jsonl', 'w') as outfile1, open('./src/ADAPET/data/CySec/' + path + '/val.jsonl', 'w') as outfile2:
    ...
    with open('./msexchange-server-cti-dataset/processed/adapet/' + path + '/test.jsonl', 'w') as outfile1, open('./src/ADAPET/data/CySec/' + path + '/test.jsonl', 'w') as outfile2:
    ...
    with open('./msexchange-server-cti-dataset/processed/adapet/' + path + '/train.jsonl', 'w') as outfile1, open('./src/ADAPET/data/CySec/' + path + '/train.jsonl', 'w') as outfile2:
